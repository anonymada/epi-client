...
export class GestureDirective implements OnInit {
  ...
  ngOnInit(): void {
    this.DIRECTIVE_GESTURES.filter((dGesture) =>
      this.gestureOpts.find(({ name }) => dGesture.name === name)
    ).map((gesture) => {
      gesture.enabled = true;
      if (gesture.name === 'swipe') {
        const swipeGestureOpts = this.gestureOpts.find(
          ({ name }) => name == 'swipe'
        );
    ...
    if (this.pressGesture.enabled && this.swipeGesture.enabled) {
      console.warn(
        'Press and Swipe should not be enabled on the same element.'
      );
    }
  ...
  handleTaps(e: any) {
    ...
    if (
      (!this.tapGesture.enabled && !this.doubleTapGesture.enabled) ||
      this.isPressing ||
      this.isMoving
    ) {
    ...
    if (isDoubleTap && this.doubleTapGesture.enabled) {
      ...
      this.tapTimeout = setTimeout(
        () => this.emitTaps(),
        this.tapGesture.interval
      );
    }
    this.lastTap = tapTimestamp;
  }

  private handleMoving(moveType: any, $event: any) {
    ...
    if (shouldReport) {
      ...
      if (
        dirArray!.includes(emitObj.dirX) ||
        dirArray!.includes(emitObj.dirY)
      ) {
